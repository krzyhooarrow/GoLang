...
func storeMachine() {
	...
	for {
		...
		case out := <-outputM:
			if storage[out.value].machineStatus {
				...
				go func() {
					...
					if rand.Float32() <= damagePropability {
						out.task.score = math.MinInt32
						pickTask(*out)
					} else {
						pickTask(*out)
		...
		case <-printM:
			fmt.Println(storage)
		case out := <-backdoor:
			storage[out].machineStatus = false
			if version == 1 {

				fmt.Println("machine ", out, " has been fixed")
			}
		}
	}
}
